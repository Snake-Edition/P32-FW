...
void screen_mbl_values_t::draw() {
    ...
    display::FillRect(GuiDefaults::RectScreen, COLOR_BLACK); //< clear the display
    for (int ix = 0; ix < GRID_MAJOR_POINTS_X; ++ix) {
        for (int iy = 0; iy < GRID_MAJOR_POINTS_Y; ++iy) {
            ...
            display::DrawText(Rect16(x, y, block_size, block_size), string_view_utf8::MakeRAM((const uint8_t *)text), resource_font(Font::normal), COLOR_BLACK, color);
        }
    }
    ...
    display::DrawText(Rect16(10, GuiDefaults::RectScreen.Height() - 40, GuiDefaults::RectScreen.Width() - 10, 20), string_view_utf8::MakeRAM((const uint8_t *)text), resource_font(Font::normal), COLOR_BLACK, COLOR_WHITE);
    snprintf(text, sizeof(text), "Y tilt %1.3f mm", static_cast<double>(y_tilt));
    display::DrawText(Rect16(10, GuiDefaults::RectScreen.Height() - 20, GuiDefaults::RectScreen.Width() - 10, 20), string_view_utf8::MakeRAM((const uint8_t *)text), resource_font(Font::normal), COLOR_BLACK, COLOR_WHITE);
}
