...
// Older (6.2) implementation, brought back from death into unit tests, to make comparisons.
...
namespace phase_stepping_old {

using phase_stepping::MotorPhaseCorrection;
using phase_stepping::opts::MOTOR_PERIOD;
using phase_stepping::opts::SIN_LUT_FRACTIONAL;
 ...
 */
int sin_lut(int x, int range = 1 << SIN_LUT_FRACTIONAL);
int cos_lut(int x, int range = 1 << SIN_LUT_FRACTIONAL);
